...
export default async function PlayPage() {
  const topScores = await Promise.all(
    ERA_LIST.map(async (era) => [era.id, await fetchTopScore(era.id)] as const),
  );
  const topScoreMap = new Map(topScores);
    ...
    <>
      ...
      <main className="min-h-screen bg-[#0a0a0a]">
        <div className="max-w-5xl mx-auto px-6 py-16">
          ...
          {/* Header */}
          <div className="mb-12">
            <h1 className="text-2xl font-semibold text-[#fafafa]">Pick a chart.</h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {ERA_LIST.map((era) => {
              ...
              const topScore = topScoreMap.get(era.id);
              return (
                <Link key={era.id} href={`/play/${era.id}`} className="group block">
                  <div className="relative h-full bg-[#111] border border-[#1e1e1e] rounded-xl p-5 flex flex-col gap-4 transition-all duration-200 group-hover:border-[#2a2a2a] group-hover:bg-[#141414]">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-[10px] font-medium tracking-wide" style={{ color: diff.color }}>
                        {diff.label}
                      </span>
                    </div>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      {topScore != null && (
                        <p
                          ...
                          style={{ color: `${era.accentColor}99` }}
                        >
