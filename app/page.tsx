...
export default async function LandingPage() {
  const [totalRuns, totalNfts] = await Promise.all([
    redis.get<number>(REDIS_KEYS.totalRuns).catch(() => null),
    redis.get<number>(REDIS_KEYS.totalNfts).catch(() => null),
  ]);
      ...
      <main className="min-h-screen bg-[#0a0a0a] text-[#fafafa]">
        ...
        <section className="relative flex flex-col items-center justify-center text-center px-6 pt-28 pb-24 overflow-hidden">
          ...
          <div className="flex items-center gap-2 mb-8">
            {(['STX', 'sBTC', 'ALEX'] as const).map((t, i) => (
              <span
                key={t}
                className="text-[10px] font-semibold px-2.5 py-1 rounded-full border"
                style={{
                  color:       (['#3b82f6','#f7931a','#8b5cf6'] as const)[i],
                  borderColor: (['#3b82f620','#f7931a20','#8b5cf620'] as const)[i],
                  background:  (['#3b82f610','#f7931a10','#8b5cf610'] as const)[i],
                }}
              >
                {t}
              </span>
          ...
          <h1
            className="text-5xl sm:text-6xl font-bold tracking-tight leading-none max-w-2xl"
          ...
          <p
            className="mt-6 text-[#555] text-lg max-w-md leading-relaxed"
          ...
          {/* Live stats */}
          {(totalRuns != null || totalNfts != null) && (
            <div className="mt-10 flex items-center gap-6 text-xs text-[#333]">
              {totalRuns != null && totalRuns > 0 && (
                <span>
                  <span className="text-[#555] font-semibold tabular-nums">{totalRuns.toLocaleString()}</span>
                  {' '}runs played
                </span>
              )}
              {totalRuns != null && totalRuns > 0 && totalNfts != null && totalNfts > 0 && (
                <span className="text-[#222]">·</span>
              )}
              {totalNfts != null && totalNfts > 0 && (
                <span>
                  <span className="text-[#555] font-semibold tabular-nums">{totalNfts.toLocaleString()}</span>
                  {' '}NFTs minted
                </span>
              )}
            </div>
            ...
            {ERA_LIST.map((era) => (
              <Link key={era.id} href={`/play/${era.id}`} className="group">
                <div className="bg-[#111] border border-[#1a1a1a] rounded-xl p-4 flex flex-col gap-3 hover:border-[#252525] hover:bg-[#131313] transition-all duration-200">
                  ...
                  <div>
                    <p className="text-[#fafafa] text-xs font-medium">{era.name}</p>
